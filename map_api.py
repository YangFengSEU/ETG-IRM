...
from data_loading.vector_map_loader import load_lane_segments_from_xml
from utils.centerline_utils import (
...
from utils.cv2_plotting_utils import get_img_contours
from utils.dilation_utils import dilate_by_l2
from utils.geometry import point_inside_polygon
...
from utils.mpl_plotting_utils import plot_lane_segment_patch, visualize_centerline
...
ROOT = "/home/lb/Downloads/interaction-dataset-master/interaction-dataset-master/tinymap"
...
class InterMap:
    ...
    def __init__(self, root: _PathLike = ROOT) -> None:
        ...
        self.city_name_to_city_id_dict = {"MA": "USA_Intersection", "GL": "USA_Intersection","EP0":"USA_Intersection","EP":"USA_Roundabout","OF":"DEU_Roundabout","FT":"USA_Roundabout","LN":"CHN_Roundabout","MT":"DEU_Merging","SR":"USA_Roundabout","ZS":"CHN_Merging"}
    ...
    def build_centerline_index(self) -> Mapping[str, Mapping[int, LaneSegment]]:
        ...
        for city_name, city_place in self.city_name_to_city_id_dict.items():
            xml_fpath = self.map_files_root / f"DR_{city_place}_{city_name}.osm"
    ...
    def build_hallucinated_lane_bbox_index(
        ...
        for city_name, city_place in self.city_name_to_city_id_dict.items():
            json_fpath = self.map_files_root / f"DR_{city_place}_{city_name}_tableid_to_lane.json"
            city_halluc_tableidx_to_laneid_map[city_name] = read_json_file(json_fpath)

            npy_fpath = self.map_files_root / f"DR_{city_place}_{city_name}_halluc_bbox_table.npy"
    ...
    def append_height_to_2d_city_pt_cloud(self, pt_cloud_xy: np.ndarray, city_name: str) -> np.ndarray:
        ...
        pts_z=np.full((pt_cloud_xy.shape[0]), 12).astype(np.float64)
        return np.hstack([pt_cloud_xy, pts_z[:, np.newaxis]])
        #return np.hstack([pt_cloud_xy, np.zeros([pt_cloud_xy.shape[0],1])])
    ...
    def get_lane_direction(
        ...
        waypoint_dists = np.linalg.norm(centerline - query_xy_city_coords, axis=1)#求范数
        #print(waypoint_dists)
        closest_waypt_indxs = np.argsort(waypoint_dists)[:2]#排列并按索引列出
        #print(closest_waypt_indxs)
        prev_waypoint_id = closest_waypt_indxs.min()
        next_waypoint_id = closest_waypt_indxs.max()
        #print(prev_waypoint_id,next_waypoint_id)
        prev_waypoint = centerline[prev_waypoint_id]
        next_waypoint = centerline[next_waypoint_id]
        #print(prev_waypoint,next_waypoint)
        
        lane_dir_vector = next_waypoint - prev_waypoint
        #print(lane_dir_vector)
        if visualize:
    ...
    def get_candidate_centerlines_for_traj(
        ...
        # Get all lane candidates within a bubble，让所有候选者在一个bubble？？
        manhattan_threshold = 2.5
        curr_lane_candidates = self.get_lane_ids_in_xy_bbox(xy[-1, 0], xy[-1, 1], city_name, manhattan_threshold)#以某种规律搜索最后一个点的车道
        ...
        while len(curr_lane_candidates) < 1 and manhattan_threshold < max_search_radius:
            ...
            curr_lane_candidates = self.get_lane_ids_in_xy_bbox(xy[-1, 0], xy[-1, 1], city_name, manhattan_threshold)#防止没找到

        assert len(curr_lane_candidates) > 0, "No nearby lanes found!!"

        # Set dfs threshold
        displacement = np.sqrt((xy[0, 0] - xy[-1, 0]) ** 2 + (xy[0, 1] - xy[-1, 1]) ** 2)#daltax平方加daltay平方
        ...
        for lane in curr_lane_candidates:
            candidates_future = self.dfs(lane, city_name, 0, dfs_threshold)#用dfs搜索出一条路，路径表达是由道路id表达的
            candidates_past = self.dfs(lane, city_name, 0, dfs_threshold, True)

            # Merge past and future
            for past_lane_seq in candidates_past:#防止搜索路径成环
        ...
        obs_pred_lanes = remove_overlapping_lane_seq(obs_pred_lanes)#去重

        # Remove unnecessary extended predecessors
        #删除通过从泳道序列中查找太多的前体而获得的泳道ID。如果任何车道id是轨迹的第一个坐标的占用车道id，则忽略所有在此之前的车道ID
        obs_pred_lanes = self.remove_extended_predecessors(obs_pred_lanes, xy, city_name)

        # Getting candidate centerlines通过之前的车道id获取lane_sequences中每个车道序列对应的中心线
        candidate_cl = self.get_cl_from_lane_seq(obs_pred_lanes, city_name)

        # Reduce the number of candidates based on distance travelled along the centerline，根据沿中心线行驶的距离减少候选车道数量
