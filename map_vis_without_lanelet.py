...
def set_visible_area(point_dict, axes):
    ...
    axes.set_xlim([945, 1050])
    axes.set_ylim([965, 1030])
...
def draw_map_without_lanelet(filename, axes, lat_origin, lon_origin):
    ...
    e = xml.parse(filename).getroot()#获取根结点
    ...
    set_visible_area(point_dict, axes)#建立画布
    ...
    for way in e.findall('way'):
        ...
        if way_type is None:
            ...
            type_dict = dict(color="darkgray", linewidth=0.5, zorder=5, dashes=[3, 3])
        elif way_type == "road_border":
            type_dict = dict(color="black", linewidth=1, zorder=20)
