...
class ngsimDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        lane_enc=np.zeros([6])
        lane_enc[int((self.D[idx,7]-1)*3+self.D[idx, 6]-1)]=1
        return hist,fut,neighbors,lane_enc
    ...
    def getHistory(self,vehId,t,refVehId,dsId):
        if vehId == 0:
            ...
            if vehTrack.size==0 or np.argwhere(vehTrack[:, 0] == t).size==0:
                ...
                #print(np.argwhere(vehTrack[:, 0] == t))
    ...
    def collate_fn(self, samples):
        ...
        lane_enc_batch = torch.zeros(len(samples),6)
        #lon_enc_batch = torch.zeros(len(samples), 2)


        count = 0
        for sampleId,(hist, fut, nbrs, lane_enc) in enumerate(samples):
            ...
            lane_enc_batch[sampleId,:] = torch.from_numpy(lane_enc)
        ...
        return hist_batch, nbrs_batch, mask_batch, lane_enc_batch, fut_batch, op_mask_batch
...
def maskedNLLTest(fut_pred, lane_pred, fut, op_mask, num_lat_classes=3, num_lon_classes = 2,use_maneuvers = True, avg_along_time = False):
    if use_maneuvers:
...
def maskedMSE(y_pred, y_gt, mask):
    acc = torch.zeros_like(mask)
    y_pred=y_pred.reshape(mask.size()[0],25,-1)
    y_gt=y_gt.reshape(mask.size()[0],25,-1)
    muX = y_pred[:,:,0]
    muY = y_pred[:,:,1]
    x = y_gt[:,:, 0]
    y = y_gt[:,:, 1]
...
def maskedMSEirm(y_pred, y_gt, mask):
    acc = torch.zeros_like(mask)
    muX = y_pred[:,:,0]
    muY = y_pred[:,:,1]
    #print(y_gt.size())
...
def maskedMSEides(generated_dest, dest,dest_mask):
    acc = torch.zeros_like(dest_mask)
    ...
    acc = acc*dest_mask
    #print(acc.size())
    lossVal = torch.sum(acc)/torch.sum(dest_mask)
    return lossVal

def maskedMSEpenalty(out, y_gt, mask):
    acc = torch.zeros_like(mask)
...
def maskedMSEdestpenalty(out, y_gt, mask):
    acc = torch.zeros_like(mask)
